...
function pl.CountPlaylists(parent, findTarget, playlistState, parentName)
  ...
  ----   iterating through all tracks in a playlist folder this two functions are combined here.       ]]-- 
...
function pl.listenToPlaylist(playlist, parent, playlistState)
  ...
  r.SetOnlyTrackSelected(playlist)
  ...
  if isParentSending == 0 and playlistState == 0 then
    ...
    r.SetMediaTrackInfo_Value(parent, "I_SOLO", 0)
    ...
    r.SetMediaTrackInfo_Value(playlist, "B_MUTE", 0)
    r.SetMediaTrackInfo_Value(playlist, "I_SOLO", 1)
    r.SetMediaTrackInfo_Value(playlist, "B_MAINSEND", 1)
    
  -- Case 2: Playlist was selected and it is currently in listening mode. --
  elseif isParentSending == 1 and playlistState == 0 then
    r.SetOnlyTrackSelected(parent)
    r.Main_OnCommand(pl.selectOnlyChildren, 0)
    r.SetMediaTrackInfo_Value(parent, "I_SOLO", 0)
    ...
    r.SetMediaTrackInfo_Value(playlist, "B_MAINSEND", 0)
    
  -- Case 3: Playlist folder parent or clearAllListening was selected --
  elseif playlistState == 1 then
    r.SetOnlyTrackSelected(parent)
    r.SetMediaTrackInfo_Value(playlist, "B_MAINSEND", 1)
    r.Main_OnCommand(pl.selectOnlyChildren, 0)
